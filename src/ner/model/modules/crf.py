...
def allowed_transitions(constraint_type: str,
    ...
    if constraint_type == "BIOUL":
        for i, (from_bioul, *from_entity) in tokens.items():
            for j, (to_bioul, *to_entity) in tokens.items():
                is_allowed = any([
                    ...
                    from_bioul in ('O', 'E', 'S') and to_bioul in ('O', 'B', 'S'),
                    # B-x can only transition to I-x or L-x
                    # I-x can only transition to I-x or L-x
                    from_bioul in ('B', 'I') and to_bioul in ('I', 'E') and from_entity == to_entity
                ])
        ...
        for i, (to_bioul, *to_entity) in tokens.items():
            if to_bioul in ('O', 'B', 'S'):
                allowed.append((start_tag, i))

        # end transitions
        for i, (from_bioul, *from_entity) in tokens.items():
            if from_bioul in ('O', 'E', 'S'):
...
class ConditionalRandomField(torch.nn.Module):
    ...
    def _joint_likelihood(self,
        ...
        for i in range(sequence_length - 1):
            ...
            # print(sequence_length, i, current_tag, next_tag)
            # if any(torch.isnan(score)):
                # print('NAN: ', transition_score, transition_score.shape, broadcast_transitions, broadcast_transitions.shape, tags.shape)
    ...
    def forward(self,
        ...
        score = torch.sum(log_numerator - log_denominator)
        return score
