...
class DataLoader(object):
    ...
    def load_data(self, types, labels=True):
        """
    ...
    def batch_iterator(self, data, params, shuffle=False, sort_by_legth=True):
        ...
        # one pass over data
        num_steps = math.ceil(data['size'] / params.batch_size)
        for i in range(num_steps):
            # fetch sentences and tags
            from_idx = i * params.batch_size
            to_idx = (i + 1) * params.batch_size
            if to_idx >= len(order):
                to_idx = len(order)
            batch_sentences = [data['data'][idx] for idx in order[from_idx:to_idx]]
            batch_tags = [data['labels'][idx] for idx in order[from_idx:to_idx]]
