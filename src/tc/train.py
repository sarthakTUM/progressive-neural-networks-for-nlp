...
def train_and_evaluate(model,
    ...
    best_acc = 0.0
    patience = 0
    early_stopping_metric = 'binary_f1'
...
if __name__ == '__main__':
    ...
    pretrained_model_dir = '/media/sarthak/HDD/TUM/Thesis/thesis-sarthak/src/tc/experiments/causaly'
    all_layer = True
    ...
    params.cuda = torch.cuda.is_available()  # set CUDA if available
    ...
    # 5.2 load data
    k_fold = None  # whether to perform k_fold validation
    combine_train_dev = False  # whether to combine train and dev for training
    train_on_dev = False  # whether to train on dev after network is converged on train
    data_loader = DataLoader(params,
                             args.data_dir,
                             data_encoder,
                             label_encoder)
    if k_fold:
        # looks for data in the respective split folders if k_fold validation
        ...
        # else takes the original data directory
    ...
    for split_dir in splits_dir:
        ...
        if k_fold:
            # pick the model corresponding to split
        ...
        ######################################################################################################
        # 6. Modeling
        # 6.1 Define the model and optimizer
        ...
        # load the pre-trained model. Uncomment below to load the pretrained weights and fine-tune them
        """
        logging.info('loading pretrained model')
        utils.load_checkpoint(os.path.join(pretrained_model_dir, 'best.pth'), model)
        if not all_layer:
            # if only last layer has to be fine-tuned
